...
# Automatically create all necessary directories (histories, model_checkpoints, model)
for path in [args.history_path, args.model_path, "model/best_model.pt"]:
...
def train_model(model, dataloaders, criterion, optimizer, args, start_epoch=1, num_epochs=25):
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        # Default initialization to avoid undefined variables in case any phase isn't run
        train_loss = val_loss = 0
        train_accuracy = val_accuracy = 0
        train_f1 = val_f1 = 0
        train_precision = val_precision = 0
        train_recall = val_recall = 0
        ...
        # Ensure both train and val have history before appending the epoch
    ...
    best_model_path = f"model/best_model_{timestamp}.pt"
    os.makedirs(os.path.dirname(best_model_path), exist_ok=True)

    # Save the model weights with the highest validation accuracy
    torch.save(best_model_wts, best_model_path)
    print(f'✅ Best model (based on val acc) saved to {best_model_path}')

    # Load the best model weights for further evaluation (optional)
...
if __name__ == "__main__":
    ...
    if args.continue_epoch:
        try:
            ...
            print(f"⚠️ No model found at {args.model_path} - starting from scratch.")

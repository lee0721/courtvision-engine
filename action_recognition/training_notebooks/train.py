...
# ✅ 自動建立所有會用到的資料夾（histories、model_checkpoints、model）
...
def train_model(model, dataloaders, criterion, optimizer, args, start_epoch=1, num_epochs=25):
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        # ✅ 放這裡確保 train/val 都有紀錄才 append epoch
    ...
    # 儲存的是 best_model_wts（val acc 最高的）
    torch.save(best_model_wts, best_model_path)
    print(f'✅ Best model (based on val acc) saved to {best_model_path}')

    # 同時載入這個權重給目前的 model（可省略，但若後續要用 test accuracy 評估就需要）
